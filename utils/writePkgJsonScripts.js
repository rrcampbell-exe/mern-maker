...
export const writePkgJsonScripts = (dirName, isOnlyFE, isFEandServer) => {
  ...
  let envFilePath
  isOnlyFE ? envFilePath = `./dist/${dirName}.env` : envFilePath = `./dist/${dirName}/client/.env`
...
`);
    ...
    setTimeout(() => {
      if (isFEandServer) {
        console.log(
          `The end! Your React front end and Express server project template can be found in ./dist/${dirName}/. Copy this directory to your preferred location on your local device and, from its root directory, use shell command npm run develop to test it out!`
        );
      } else {
        console.log(
          `The end! Your full stack MERN project template can be found in ./dist/${dirName}/. Copy this directory to your preferred location on your local device and, from its root directory, use shell command npm run develop to test it out!`
        );
      }
    }, 1000);
  });
